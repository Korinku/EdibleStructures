...
public class Shipwreck extends EnchantedItem {

	public Shipwreck(Settings settings) {
		super(settings);
	}
		...
		if (this.isFood()) {
			if (user instanceof PlayerEntity) {
				...
				player.getInventory().setStack(PlayerInventory.OFF_HAND_SLOT,
						new ItemStack(ModItems.CANNON_ARM));
			}
	...
	public void appendTooltip(ItemStack itemStack, World world, List<Text> tooltip, TooltipContext tooltipContext) {
		tooltip.add(new TranslatableText("item.estr.shipwreck").formatted(Formatting.DARK_PURPLE));
	}
